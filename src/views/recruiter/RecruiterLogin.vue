<template>
  <div class="login-wrapper">
    <div v-if="isLogin" style="background-color: #fff;width: 100%;text-align: center;">
      <h1 style="margin-top: 0.5em;">应聘企业登录</h1>
    ...
    <div v-if="!isLogin" style="background-color: #fff;width: 100%;text-align: center;">
      <h1 style="margin-top: 0.5em;">应聘企业注册</h1>
        ...
        if (!err) {
          ...
          userApi.login.r(loginVo).then(resp => {
            if (resp.data.code === "000000") {
              setToken("x-auth-token-recruiter", resp.data.data.token);
              setItem("recruiter-username", resp.data.data.username);

              Bus.$emit("recruiterUsername", resp.data.data.username);
              this.$router.replace({
                name: "recruiterJob"
              });
            }
